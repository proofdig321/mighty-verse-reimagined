...
async function expectPublicHolographic(page: import("@playwright/test").Page) {
  ...
  await expect(page.getByRole("link", { name: /Return to Universe/i })).toBeVisible();
...
test("dashboard Experience discovers Super Hero Ego 2.5D without mutating production", async ({
  ...
  reportEvidence(testInfo, "BROWSER VERIFIED", "Stage 4.8 Experience architecture", page.url(), notes, observe);
});
...
test("Mural, Scene, and Creative Moment surfaces enter the same Universe Experience", async ({ page, observe }, testInfo) => {
  ...
  reportEvidence(testInfo, "BROWSER VERIFIED", "Reveal surfaces share Universe Experience", page.url(), [
    "Mural, Scene inspect, and Creative Moment identity all enter the same holographic Experience",
    "no separate Scene or Moment 2.5D routes",
  ], observe);
});
