...
export function StudioCreationSurface({
  ...
  const hasRefs =
    references.filter((r) => r.still_url).length > 0 || workFrames.length > 0;
  const hasStill = Boolean(selected?.still);
  const hasMotion = Boolean(selected?.endpoint);

  // Determine available generation modes from capability + inputs
  ...
  return (
    <div className="studio-creation-surface flex flex-col gap-0 min-h-0">
      {/* ── Result / Preview area ── */}
      <div className="studio-result-area">
        {hasMotion && selected?.endpoint ? (
          ...
          <div className="relative aspect-video w-full rounded-lg overflow-hidden bg-muted/20">
            ...
            />
            {selected.title && (
              <div className="absolute bottom-0 inset-x-0 px-3 py-2 bg-gradient-to-t from-background/80 to-transparent">
          ...
          <div className="relative aspect-video w-full rounded-lg bg-muted/20 flex items-center justify-center">
          ...
          <div className="aspect-video w-full rounded-lg bg-muted/10 flex items-center justify-center">
            <p className="text-sm text-muted-foreground">
              Select a panel or create one to begin
            </p>
          </div>
        )}
          ...
          (selectedJob.status === "queued" ||
            ...
            <div className="mt-2 flex items-center gap-2">
              <span
                ...
                data-generation-status={selectedJob.status}
              >
        ...
        {selectedJob &&
          (selectedJob.status === "failed" ||
            selectedJob.status === "unavailable" ||
            selectedJob.status === "blocked") && (
            <div className="mt-2 flex items-center gap-2">
              <span className="text-xs text-destructive">
                {
                  operatorGenerationMessage(selectedJob.error?.message)
                    .operator
                }
              </span>
      ...
      {/* ── Sentinel Advisory ── */}
      {hasSentinel && (
        <div className="studio-sentinel-advisory mt-3">
          <button
            type="button"
              ...
              {selectedObservation && (
                <div>
                  ...
                  <dl className="grid gap-1.5 text-xs">
                    <div className="flex gap-2">
                      ...
                      <dd className="font-mono text-foreground">
                        {formatTimelineMs(selectedObservation.start_ms)} –{" "}
                        {formatTimelineMs(selectedObservation.end_ms)}
                      </dd>
                    </div>
                    ...
                    {selectedObservation.camera && (
                      <div className="flex gap-2">
                        ...
                        <dd className="text-foreground">
                          {selectedObservation.camera_explanation ||
                            selectedObservation.camera}
                          {selectedObservation.framing
                            ? ` · ${selectedObservation.framing}`
                            : ""}
                        </dd>
                      </div>
                  ...
                  <p className="mt-2 text-[10px] text-muted-foreground">
                    Sentinel observes. It does not author creative meaning.
                  </p>
                        ...
                        onDraftChange({
                          ...
                          generation_metadata: {
                            ...
                            transformation_instruction: [
                              editorPanel.generation_metadata
                                ?.transformation_instruction,
                              selectedObservation.action,
                            ]
                              .filter(Boolean)
              ...
              {selectedFrame && (
                <div>
                  ...
                  <p className="text-xs text-foreground">
                    {selectedFrame.source_title} ·{" "}
                    {formatTimelineMs(selectedFrame.timestamp_ms)}
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="mt-3">
          ...
          >
            ...
            <span className="text-[9px] normal-case tracking-normal font-normal">
              {references.filter((r) => r.still_url).length +
                workFrames.length}{" "}
              attached
            </span>
          </button>
          {refsOpen && (
            <div className="flex flex-wrap gap-2">
              {workFrames.map((frame, i) => (
                <div
                  key={i}
                  className="relative w-16 rounded overflow-hidden border border-border/60 group"
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={frame.still_url}
                  ...
                  />
                  <p className="px-1 py-0.5 text-[9px] text-muted-foreground truncate bg-card/80">
                    {frame.source_title}
                  </p>
                </div>
              ))}
              {references
                .filter((r) => r.still_url)
                .map((ref) => (
                  <div
                    key={ref.asset_id}
                    className="relative w-16 rounded overflow-hidden border border-border/60"
                  >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={ref.still_url!}
            ...
            <div className="space-y-2">
              <details className="text-xs">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {(
                    ...
                    <div key={field}>
                      <label
                        htmlFor={`panel-detail-${field}`}
                        className="block text-[10px] uppercase tracking-[0.12em] text-muted-foreground mb-0.5 capitalize"
                      >
                        {field.replace("_", " ")}
                      </label>
                      <input
              ...
              {selectedPersisted.user_locked && (
                <p className="text-[10px] text-muted-foreground">
                  Authored — AI will not overwrite this panel silently.
                </p>
              )}
            </div>
          )}
      ...
      {/* ── Directive + Generation composer ── */}
      <div className="studio-composer mt-4">
        <Textarea
        ...
        <div className="studio-composer-bar mt-3">
          <div className="studio-composer-controls">
            ...
            <select
              ...
              value={aspectRatio}
              onChange={(e) =>
                onSetAspect(e.target.value === "9:16" ? "9:16" : "16:9")
              }
            >
            ...
            {/* Duration — only when video is available */}
            {canTextToVideo && (
              <select
                aria-label="Duration"
                className="h-7 rounded-full border border-border bg-background px-2.5 text-xs text-foreground"
                ...
                onChange={(e) =>
                  onSetDuration(Number(e.target.value) as 4 | 6 | 8)
                }
              >
          ...
          <div className="flex items-center gap-2">
            ...
            {canTextToVideo && (
              <Button
                ...
                title={motionReady.reason ?? "Generate"}
            ...
            {!canTextToVideo && !canGenerateStill && (
              <Button
                ...
                size="sm"
                disabled={!stillReady.available}
                title={stillReady.reason ?? "Generate"}
                onClick={onGenerateStill}
                className="h-8 gap-1.5"
              >
                <Sparkles size={13} />
        ...
        {/* Image-to-video when still is available */}
          ...
          <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
            <div className="flex items-center gap-1.5">
              ...
              <img
                src={selected.still}
                alt=""
                className="h-6 w-10 rounded object-cover border border-border/60"
              />
            ...
            <Button
              ...
              className="h-6 text-[10px]"
              onClick={() =>
                onEnqueue("animate-still", { still_url: selected.still })
              }
            >
              Use still
            </Button>
        ...
        {mediaState.status !== "idle" &&
          ...
          mediaState.status === "unavailable" ||
          mediaState.status === "blocked" ? (
            ...
            <p
              className="mt-2 text-xs text-muted-foreground animate-pulse"
              data-generation-status={mediaState.status}
            >
              {mediaState.message}
            </p>
          ))}
