...
function CardBack() {
  return (
    ...
    >
      ...
      <circle cx="80" cy="110" r="44" fill="none" stroke="oklch(0.72 0.22 290 / 30%)" strokeWidth="0.75" />
      <circle cx="80" cy="110" r="36" fill="none" stroke="oklch(0.72 0.22 290 / 40%)" strokeWidth="0.75" />
      <polygon
      ...
      <polygon
        ...
        strokeWidth="1"
      />
      <circle cx="80" cy="110" r="4" fill="oklch(0.78 0.15 75 / 85%)" />
...
export default function SceneDeck({
  ...
  const controls = (
    <div className="flex items-center gap-2 shrink-0">
      <Button
        ...
        size="sm"
        onClick={() => {
          setItems((current) => shuffleItems(current));
          // reset reveals on shuffle for gamification
          if (faceDownUntilSelected) {
    ...
    return (
      ...
      >
        ...
        <span
          ...
          style={{
            background: isActive ? "var(--accent-mv-gold)" : "color-mix(in oklch, var(--accent-mv) 30%, var(--background))",
            color: isActive ? "#000" : "var(--accent-mv)",
            border: `1px solid ${isActive ? "var(--accent-mv-gold)" : "color-mix(in oklch, var(--accent-mv) 50%, transparent)"}`,
          }}
        >
  ...
  // ── Deck view card ─────────────────────────────────────────────────────────
  function deckCard(scene: SceneDeckItem, index: number) {
    const isRevealed = revealedIds.has(scene.id);
    const isActive = activeId === scene.id;
    ...
    const card = (
      <div
        ...
        draggable
        onClick={() => {
          if (!draggedInteraction.current) reveal(scene.id);
        }}
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") { e.preventDefault(); reveal(scene.id); }
        }}
    ...
    <section aria-labelledby="scene-deck-heading">
      ...
      {!hideHeader && (
        <div className="flex items-start justify-between gap-4 mb-5">
          ...
          <div className="pt-1">{controls}</div>
        </div>
      )}

      {hideHeader && (
        <div className="flex items-center justify-end gap-2 mb-5">{controls}</div>
      )}
