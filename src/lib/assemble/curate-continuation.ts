...
export function curateContinuation(input: {
  ...
  actions.push({ href: creativeSuiteHref(universeId, "curate"), label: "Open Creative Suite" });
  if (input.assetId) {
  ...
  if (input.muralRegistered && !input.mediaAttached && !input.assetId) {
    return {
      copy: "Mural registered. Media is not attached. Next: associate incoming media, then inspect, then continue in Creative Suite for storyboard, animation planning, and 2.5D Preview.",
      actions: [
        { href: "/authority/curate", label: "Incoming media" },
        { href: creativeSuiteHref(universeId, "curate"), label: "Open Creative Suite" },
      ],
    };
  }
  ...
  if (input.associated || input.mediaAttached) {
    return {
      copy: "Associated. Next: inspect the media, review Sentinel evidence, then continue in Creative Suite. Storyboard, animation planning, and 2.5D Preview live in the Suite — not as a public Experience substitute.",
      actions,
    };
  }

  return {
    copy: "This Universe is the canonical work. Inspect associated media, then continue in Creative Suite to assemble storyboard, animation, authorisation, and 2.5D Preview. Experience remains separate.",
    actions,
  };
}
