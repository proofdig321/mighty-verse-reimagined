...
| Scene object | `scene-object.tsx` | SuiteScene | identity + timing + still + order + presence | yes | open record | consumes title + still + window | yes | Face-up, numbered, known. Not facedown. Not a timeline dashboard. |
...
| Scene still | `scene-artwork.ts` + `scene-artwork-authoring.tsx` + `POST /api/authority/media/artwork` | HTTPS thumbnail on `work_presentation.artwork_asset_id` | artwork-only; mural-frame shortcut | yes | yes | stills | yes | Stage 4.5. Does not create Scenes or windows. |
| Scene timing | `scene-timing.ts` + `scene-timing-authoring.tsx` + `PATCH /api/authority/media/timeline` | binding `start_ms`/`end_ms` | shapes existing window only | yes | yes | stills + playback windows | yes | Stage 3.5. Compact start/end on the Scene object. Sentinel does not create Scenes. Visible even without a binding. |
