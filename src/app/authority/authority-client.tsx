...
export default function AuthorityClient() {
  ...
  const [editingRightsBindingId, setEditingRightsBindingId] = useState<string | null>(null);
  const [rightsHolderRef, setRightsHolderRef] = useState("");
  const [rightsBasis, setRightsBasis] = useState("");
  ...
  async function saveRights(record: WorkRecord) {
    if (!record.binding || !rightsHolderRef || !rightsBasis.trim()) return;
    await act("Rights update", "/api/authority/media/rights", { binding_id: record.binding.binding_id, master_id: record.master.master_id, rights_holder_ref: rightsHolderRef, rights_basis: rightsBasis }, { workTitle: titleFor(record) });
    setEditingRightsBindingId(null);
  }
  ...
  return (
    <div className="min-h-screen bg-muted/30 pt-12 lg:flex lg:pt-0">
      ...
      {selected?.binding && !selected.status.rightsVerified && editingRightsBindingId !== selected.binding.binding_id && <Button size="sm" variant="outline" onClick={() => { setEditingRightsBindingId(selected.binding!.binding_id); setRightsHolderRef(""); setRightsBasis(""); }}>Review video rights</Button>}
      {selected?.binding && editingRightsBindingId === selected.binding.binding_id && <Card><CardContent className="space-y-3 pt-4"><h3 className="text-sm font-semibold">{titleFor(selected)} — Video rights</h3><select value={rightsHolderRef} onChange={event => setRightsHolderRef(event.target.value)} className="border-input bg-background w-full rounded-md border px-3 py-2 text-sm"><option value="">Select rights owner</option>{participants.map(participant => <option key={participant.participant_id} value={participant.participant_id}>{participant.label}</option>)}</select><input value={rightsBasis} onChange={event => setRightsBasis(event.target.value)} placeholder="Rights basis" className="border-input bg-background w-full rounded-md border px-3 py-2 text-sm" /><div className="flex gap-2"><Button size="sm" onClick={() => void saveRights(selected)}>Save rights</Button><Button size="sm" variant="outline" onClick={() => setEditingRightsBindingId(null)}>Cancel</Button></div></CardContent></Card>}
